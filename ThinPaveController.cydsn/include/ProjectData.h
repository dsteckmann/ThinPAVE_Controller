...
{
  uint16_t   density1_count;            //2 density sys 1 count at station         
  uint16_t   density2_count;            //2  density sys 2 count at station         
  uint16_t   DT[PROFILE_DENSITIES];     //30   Profile mode has 15 densities        
  uint16_t   density;                   //2
  uint16_t   density1_stand;            //2 d1_stand when station count was done    
  uint16_t   density2_stand;            //2 d2_stand when station count was done       
  float      MA;                        //4 MA count when station count was done     
  float      den_off;                   //4                                          
  float      thickness;                 //4                                          
  float      battery_voltage[2];        //4
  uint8      mode;                      //1 Metal, Normal, Profile  
  uint8_t    units;                     //1                                          
  char       name[PROJ_NAME_LENGTH];    //16 name of station                 
  date_time_t date;                     //7
  uint32_t   ser_number;                //4
  float      version;                   //4
  GPSDATA    gps_read;                  //13
 } station_data_t;  // 102 bytes                                                    
