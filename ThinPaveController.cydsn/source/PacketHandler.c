...
#define NUM_CMDS 4
const uint8 cmd_arry[NUM_CMDS] = { CMD_REMOTE_START,CMD_REMOTE_RECALL,CMD_SET_CC,CMD_SEND_CC };
 ...
 void checkForPacket ( uint8 c ) {
   switch ( STATE ) {
    ...
    case STATE_WAITING:
		...
		else { STATE = STATE_CHECKING; }    // BEGIN_FLAG_0 was received. If a valid packet, BEGIN_FLAG_1 will be next
		break;
	  case STATE_CHECKING:                      // Look for the second byte of the synch frame
		...
		case STATE_COLLECT_PAYLOAD:
            ...
            if (packet_ptr > 450 ) {            // If the payload goes over 450 bytes, something bad happened, so reset.
                STATE = STATE_WAITING;          // None of the packets should be over 400 bytes, so I picked 450 to reset
			    PacketStarted = false;   
                PacketProcessed = true;
            }  
			break;
		...
		case STATE_GET_CS_B:  
			cs_b = c;
			if (( cs_a ==  calc_cs_a) && ( cs_b ==  calc_cs_b) ) // all the packet has been received. CHeck that the CS is good.
			{
