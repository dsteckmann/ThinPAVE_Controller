...
void serial_number ( Bool source  ) { //Enter or Edit Serial Number 0 is gauge, 1 is computer
    ...
    if(source){
        while(1)
        {
            ...
            //delay_ms(500);
            _UART2_PRINT ("Enter the 4 digit operator access code <Enter>\n\r"); //RGS uncomented this
            delay_ms(100);
            gets_comp(num);
            if(atol(num) == ACCESS_CODE)
            {
                _UART2_PRINT ( "Serial Number: "); //RGS uncomented this
                delay_ms(100);
                _UART2_PRINTF ("%lu\n\r", serial_number); //RGS uncomented this
                delay_ms(100);
                ...
                if (getRXNumEntries(&s_Computer_RX) > 0 )
                {
                    if (waitc_RX(&s_Computer_RX, 600000) == 'Y')                   //change serial # via serial port interface
                    {
                        ...
                        gets_comp(num);
                        //delay_ms(500);
                        // needs to be 5 digit number  RGS Why?, no checks here, CalPlus ignores the 5 digit limit
                        serial_number = atol(num);
                        snprintf(num,9,"%ld",serial_number);
                        _UART2_PRINTF ( "New Serial Number: %s\n\r", num);        //RGS uncomented this
